      ...
      <Card className="bg-blue-50 border-blue-200">
        <CardContent className="pt-4">
          <p className="text-sm text-blue-800">
            <strong>Broj računa:</strong> Prema hrvatskim propisima o fiskalizaciji, broj računa
            mora biti u formatu{" "}
            <code className="bg-blue-100 px-1 rounded">broj-poslovni_prostor-naplatni_uređaj</code>{" "}
            (npr. 43-1-1).
          </p>
        </CardContent>
      </Card>
        ...
        {premises.length === 0 ? (
          <Card>
            <CardContent className="py-6">
              <EmptyState
                ...
                description="Poslovni prostor je obavezan za fiskalizaciju računa. Svaki prostor ima jedinstveni kod koji se koristi u broju računa. Koristite obrazac iznad za dodavanje."
              />
            </CardContent>
          </Card>
        ) : (
          premises.map((p) => (
            <Card key={p.id} className={p.isDefault ? "border-green-500" : ""}>
              ...
              <CardContent>
                <DevicesList premisesId={p.id} companyId={company.id} devices={p.devices} />
              </CardContent>
            </Card>
          ))
        )}
