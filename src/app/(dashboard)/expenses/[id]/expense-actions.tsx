...
export function ExpenseActions({ expense }: { expense: Expense }) {
  ...
  return (
    <div className="flex gap-2">
      {canPay && (
        <select
          onChange={(e) => { if (e.target.value) handlePay(e.target.value as PaymentMethod) }}
      ...
      <Button variant="destructive" onClick={handleDelete} disabled={isLoading}>Obriši</Button>
    </div>
  )
}
