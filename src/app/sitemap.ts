...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticEntries: MetadataRoute.Sitemap = sitemapRoutes.flatMap(
    ([, routeDef]) => {
  ...
  const comparisonEntries: MetadataRoute.Sitemap = comparisonSlugs.map(
    (slug) => ({
  ...
  const publishedPosts = await drizzleDb
    ...
    .from(newsPosts)
    .where(
      and(eq(newsPosts.status, "published"), lte(newsPosts.publishedAt, now))
    )
