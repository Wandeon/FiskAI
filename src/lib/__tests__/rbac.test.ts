    ...
    it("should ensure delete permissions are more restrictive than update", () => {
      ...
      resources.forEach((resource) => {
        ...
        deleteRoles.forEach((role) => {
          assert.ok(updateRoles.includes(role), `${resource}: ${role} can delete but cannot update`)
        })
      })
    })
    ...
    it("should ensure all roles can read", () => {
      ...
      resources.forEach((resource) => {
        ...
        allRoles.forEach((role) => {
          assert.ok(readRoles.includes(role), `${resource}: ${role} should have read permission`)
        })
      })
    })
  })
