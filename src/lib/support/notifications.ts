...
async function getTicketRecipients(
  ...
  staffAssignments.forEach((assignment) => {
    if (assignment.staff.id !== excludeUserId && !recipientIds.has(assignment.staff.id)) {
      recipientIds.add(assignment.staff.id)
      recipients.push({
        email: assignment.staff.email,
        name: assignment.staff.name || undefined,
      })
    }
  })

  companyAdmins.forEach((admin) => {
    if (admin.user.id !== excludeUserId && !recipientIds.has(admin.user.id)) {
      recipientIds.add(admin.user.id)
      recipients.push({
        email: admin.user.email,
        name: admin.user.name || undefined,
      })
    }
  })
